        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">LIABILITY(Bankloan/Company Name)</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">LIABILITY(Bankloan/Company Name)</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
<script>
  $(document).ready(function () {
    $("#data-table").DataTable({
      "ajax":"<?=base_url('staff/Staff/get_all_LibalityBankName')?>",
      "order":[],
      "ordering":false,
     ...
     $(document).on("click","#delete-all-record",function(){
            ...
            function(isConfirm) 
            {
                    if (isConfirm) {
                      ...
                      var TN = 'liability_bankloan_company_name';
                       $.ajax({
...
<script>
    $(document).on("click","#save-record-new",function(){
        ...
        form_data.append('bank_name',name);
        if(name!="")
        {
            $("#mfc-name").removeClass("is-invalid");
            $.ajax({
            url: baseUrl + "staff/Staff/insert_LibalityBankName",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on("click","#record-edit-btn",function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/edit_LibalityBankName",
        type: "POST",
        data:form_data,
        contentType: false,
        processData: false,
          ...
          if ((responseData != null)) 
           {
             ...
             $("#mfc-name").parent().css("margin-bottom","0");
             $("#mfc-name-err").text("");  
             $("#mfc-name").val(responseData['bank_name']);
    ...
    $(document).on("click","#update-record-btn",function(){
       ...
       var baseUrl = "<?=base_url()?>";
        ...
        if(name!="")
        {
            ...
            $.ajax({
            url: baseUrl + "staff/Staff/update_LibalityBankName",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on('change','#excel-input-file',function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/upload_excel_to_LibalityBankName",
        type: "POST",
        data: form_data,
        contentType: false,
        processData: false,
...
<script>
    $(document).on("click","#record-delete-btn",function(){
       ...
       var recordid = $(this).attr("key");
        ...
        function(){
            var form_data = new FormData();
            var table = "liability_bankloan_company_name";
