        ...
        <div class="row mb-2">
          ...
          <div class="col-sm-6">
            <ol class="breadcrumb float-sm-right">
              ...
              <li class="breadcrumb-item active">EMERGENCY FUND(Wallet Name)</li>
            </ol>
          </div><!-- /.col -->
        </div><!-- /.row -->
      </div><!-- /.container-fluid -->
          ...
          <div class="col-12">
            <div class="card">
              <div class="card-header">
                <h3 class="card-title">EMERGENCY FUND(Wallet Name)</h3>
              </div>
              <!-- /.card-header -->
              <div class="card-body">
...
<script>
  $(document).ready(function () {
    $("#data-table").DataTable({
      "ajax":"<?=base_url('staff/Staff/get_all_WalletDetail')?>",
      "order":[],
      "ordering":false,
     ...
     $(document).on("click","#delete-all-record",function(){
            ...
            function(isConfirm) 
            {
                    if (isConfirm) {
                      ...
                      var TN = 'emergency_fund_wallet_name';
                       $.ajax({
...
<script>
    $(document).on("click","#save-record-new",function(){
        ...
        if(name!="")
        {
            $("#mfc-name").removeClass("is-invalid");
            $.ajax({
            url: baseUrl + "staff/Staff/insert_EF_WalletBank",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on("click","#record-edit-btn",function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/edit_EF_WalletBank",
        type: "POST",
        data:form_data,
        contentType: false,
        processData: false,
    ...
    $(document).on("click","#update-record-btn",function(){
       ...
       var baseUrl = "<?=base_url()?>";
        ...
        if(name!="")
        {
            ...
            $.ajax({
            url: baseUrl + "staff/Staff/update_EF_WalletBank",
            type: "POST",
            data: form_data,
            contentType: false,
            processData: false,
...
<script>
    $(document).on('change','#excel-input-file',function(){
       ...
       $.ajax({
        url: baseUrl + "staff/Staff/upload_excel_to_EF_WalletBank",
        type: "POST",
        data: form_data,
        contentType: false,
        processData: false,
...
<script>
    $(document).on("click","#record-delete-btn",function(){
       ...
       var recordid = $(this).attr("key");
        ...
        function(){
            var form_data = new FormData();
            var table = "emergency_fund_wallet_name";
